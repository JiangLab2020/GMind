...
`perl $Gene_Modeller/LocRegionUni.pl GeneLoc_Sort.txt`;
...
if ( !(-e "GeneLoc_Sort_uni_DB.txt") ) {
	`perl $Gene_Modeller/Blast_MultiThreads.pl $Query_DB GeneLoc_Sort_uni.pep GeneLoc_Sort_uni_DB.txt 1 $BlastNum $CPU_Num`;
}
...
foreach my $key (keys %AList) {
	...
	if ( $AlignNum > 1 ) {	
		...
		`perl $Gene_Modeller/Fasta_DelLargeDiv.pl Tem_Tem.fas Tem_Tem.txt 0`;
