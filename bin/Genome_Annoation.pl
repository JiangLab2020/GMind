...
`cp $Gene_Modeller_lib/$Query_DB $TargetDir/Query_DB.pep`;
	...
	if ( $dots[$j] =~ /\.fasta/ and $size > 0 and $dots[$j] !~ /Trim_All.fasta/ ) {
		if ( $dots[$j] !~ /\_$Pfam\.fasta/ ) {
			if (! -e "$Auga_dir/$species\_augustus.pep") {
				`perl $Gene_Modeller/Genome_augustus.pl $dots[$j] $CPU`;
				`perl $Gene_Modeller/HMM_Pfam.pl $species\_augustus.pep $Pfam $CPU`;
			...
			if (! -e "$Augr_dir/$species\_augPU.pep") {
				`perl $Gene_Modeller/Genome_augPUGT.pl $dots[$j] $CPU`;
				`perl $Gene_Modeller/HMM_Pfam.pl $species\_augPU.pep $Pfam $CPU`;
			...
			if (! -e "$Glm_dir/$species\_glimmerHMM.pep") {
				`perl $Gene_Modeller/Genome_glimmhmm.pl $dots[$j]`;
				`perl $Gene_Modeller/HMM_Pfam.pl $species\_glimmerHMM.pep $Pfam $CPU`;
		...
		if ( !(-e "$Merge_dir/$species\_uni_DB.pep") ) {
			...
			if ( @Files ) {
				...
				`perl $Gene_Modeller/Uni_GeneAnn1.pl $file`;
...
if ( !(-e "All_uni_DB_adj_div.pep") ) {
	...
	`perl $Gene_Modeller/Fasta_div.pl All_Methods.pep Query_DB.pep $CPU`;
	`cat Merge_dir/*_uni_DB.pep >All_uni_DB.pep`;

	`perl $Gene_Modeller/Adjust_div.pl`;
	`perl $Gene_Modeller/Uni_GeneAnn2.pl All_uni_DB_adj_div.txt All_uni_DB_adj.pep 0.2`;
}
