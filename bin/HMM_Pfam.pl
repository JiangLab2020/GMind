...
####   此程序用于多线程过滤NR库(198058131)中的植物序列(8080288)
####   Hmmer
...
my $InputPep = $ARGV[0];     ###    输入蛋白序列
my $Pfam     = $ARGV[1];     ###    输入蛋白结构域
...
######################################## 1.HMM $Pfam #################################
print "################# 1.HMM $Pfam #################","\n";
`perl $Gene_Modeller/Hmmer_MultiThreads.pl $InputPep $CPU $Pfam`;
my %AList=();
...
######################################## 2.HMM ALL #################################
print "################# 2.HMM ALL #################","\n";
`perl $Gene_Modeller/Hmmer_MultiThreads.pl $Target1 $CPU`;
my %BList=();
