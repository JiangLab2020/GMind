...
`perl $Gene_Modeller/Tab_Rank.pl $Tem_Gff1 $Tem_Gff2 1 9 4`;

my @Gene  = ();             ###  记录每个基因的ID
my %GList = ();             ###  记录每个基因CDS拼成的序列
my %SList = ();             ###  记录每个基因的Strand
...
#`rm "$Target\_glimmerHMM.gff"`;
#`rm "$Target\_glimmerHMM_Adj.cds"`;
#####################################################################################################################################################
