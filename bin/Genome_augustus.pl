...
my $Group   =  $ARGV[1];                     ####  需要将查询的Fasta文件分成多少组                   
...
for (my $j=0;$j<$Group ;$j++) {
	...
	if ($pid) {                        #   parent
		...
		`augustus --species=arabidopsis $species\_$j\.fas >$species\_$j\.gff`;
...
open OUTPUT1,">$species\_augustus.pep";
open OUTPUT2,">$species\_augustus.cds";
open OUTPUT3,">$species\_augustus.gff";
for (my $j=0;$j<$Group ;$j++) {
	my $str="";
	my $strCDS="";
	my $strStrand="";
...
#`mv $species\.pep /public/home/chengj/3_LiuYuQian/Genome_Annotation/genome/genome_augustus/$species\_augustus.pep`;
#`mv $species\.cds /public/home/chengj/3_LiuYuQian/Genome_Annotation/genome/genome_augustus/$species\_augustus.cds`;
########################################################################################################
...
sub FetchLocSeq{
	my ($MySeq,$a1,$a2,$Strand) = @_;    ### 分别对应：  输入序列，输入起始坐标，输入结束坐标，输入链的“+”或者“-”。
	my $str1="";
	if ( $a1 > 0 ) {
