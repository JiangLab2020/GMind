...
if ( !(-e "$OutFlag\.txt") ) {
	`perl $GMind/Blast_MultiThreads.pl $InputPep $InputFas $OutFlag\.txt 3 $BlastNum $CPU_Num`;
}

`perl $GMind/Blast_Uni.pl $OutFlag\.txt`;
...
foreach my $key (sort keys %DList) {
	...
	if ( ($MaxGene-$MinGene) > $MaxGeneLen*0.6 ) {
		...
		for (my $i=0;$i<@LocRem ;$i++) {
			...
			if ( $GapMark[$i] eq "UTR" ) {
				if ( ($i == 0 and $#LocRem > 0) or ($#LocRem == 0 and $b[0] == 1) ) {               
					if ( $LenLoc > $MaxUtrLen ) {
			...
			if ( $GapMark[$i] eq "Intron" ) {				
				if ( $LenLoc > $MaxIntronLen and $LenGene < 5 ) {           
